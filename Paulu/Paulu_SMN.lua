...
--]]
    ...
    pacts.sleep = {['Shiva']='Sleepga', ['Diabolos']='Nightmare', ['Cait Sith']='Mewing Lullaby', ['Ramuh']='Volt Strike'}
	...
	pacts.atk = {['Ifrit']='Flaming Crush', ['Shiva']='Rush', ['Garuda']='Predator Claws', ['Titan']='Mountain Buster',
        ['Ramuh']='Chaotic Strike', ['Leviathan']='Spinning Dive', ['Carbuncle']='Poison Nails', ['Fenrir']='Eclipse Bite',
        ['Diabolos']='Blindside', ['Cait Sith']="Regal Gash"}

	pacts.nuke = {['Ifrit']='Meteor Strike', ['Shiva']='Heavenly Strike', ['Garuda']='Wind Blade', ['Titan']='Geocrush',
        ['Ramuh']='Thunderstorm', ['Leviathan']='Grand Fall', ['Carbuncle']='Holy Mist', ['Fenrir']='Impact',
        ['Diabolos']='Night Terror', ['Cait Sith']='Level ? Holy'}
		
	pacts.curaga = {['Ifrit']='', ['Shiva']='Sleepga', ['Garuda']='Whispering Wind', ['Titan']='',
        ['Ramuh']='Thunderspark', ['Leviathan']='Spring Water', ['Carbuncle']='Healing Ruby II', ['Fenrir']='Heavenward Howl',
        ['Diabolos']='Somnolence', ['Cait Sith']='Mewing Lullaby'}
		
	pacts.buff1 = {['Ifrit']='Crimson Howl', ['Shiva']='Crystal Blessing', ['Garuda']='Hastega II', ['Titan']='Earthen Ward',
        ['Ramuh']='Rolling Thunder', ['Leviathan']='Soothing Current', ['Carbuncle']='Shining Ruby', ['Fenrir']='Ecliptic Howl',
        ['Diabolos']='Noctoshield', ['Cait Sith']='Reraise II'}
		
	pacts.buff2 = {['Ifrit']='Inferno Howl', ['Shiva']='Frost Armor', ['Garuda']='Aerial Armor', ['Titan']='Earthen Armor',
        ['Ramuh']='Lightning Armor', ['Leviathan']='Slowga', ['Carbuncle']='Pacifying Ruby', ['Fenrir']='Ecliptic Growl',
        ['Diabolos']='Dream Shroud', ['Cait Sith']='Raise II'}
		
	pacts.debuff = {['Ifrit']='Conflag Strike', ['Shiva']='Diamond Storm', ['Garuda']='Fleet Wind', ['Titan']='Crag Throw',
        ['Ramuh']='Shock Squall', ['Leviathan']='Tidal Roar', ['Carbuncle']='Soothing Ruby', ['Fenrir']='Lunar Roar',
        ['Diabolos']='Pavor Nocturnus', ['Cait Sith']='Eerie Eye'}
--		
...
bp_magicalTP=S{		'Geocrush','Grand Fall','Wind Blade','Heavenly Strike','Thunderstorm','Meteor Strike', 'Healing Ruby', 'Whispering Wind', 'Spring Water','Healing Ruby II'}
...
bp_buff=S{		'Shining Ruby','Frost Armor','Rolling Thunder','Crimson Howl','Lightning Armor','Ecliptic Growl','Hastega','Noctoshield',
				...
				'Soothing Current','Hastega II','Crystal Blessing', 'Raise II','Aerial Armor','Reraise II', 'Glittering Ruby','Earthen Ward'}
...
function set_macros(sheet,book)
    if book then 
        send_command('@input /macro book '..tostring(book)..';wait 1;input /lockstyleset 1;wait 1;input /macro set '..tostring(sheet))
		return
    end
...
savedIdle = '0'
 ...
 function precast(spell)
    ...
    if buffactive['Astral Conduit'] and pet.isvalid then
        return
    end
    ...
    elseif spell.type == 'SummonerPact' then
        ...
        if spell.name == 'Dark Spirits' then
    ...
    elseif spell.type == 'WhiteMagic' or spell.type == 'BlackMagic' or spell.name == AvatarList:contains(spell.name) or spell.type == 'Trust' then
    ...
    if (spell.type == 'BloodPactRage' or spell.type == 'BloodPactWard') then
        ...
        elseif bp_hybrid:contains(spell.name) then
            ...
            if pet.name == 'Ifrit' then
               -- equip({rring="Varar Ring"})--[[Change rring to lring if you put Evoker's on your right hand]]
            end
		...
		elseif bp_magicalTP:contains(spell.name) then
         
            equip(sets.avatar.mabtp)
...
function playerIdle(savedIdle)
	if savedIdle == '0' then
		idle(pet)
	elseif savedIdle == '1' then
		equip(sets.me.refresh)
	elseif savedIdle == '2' then
		equip(sets.me.pdt)
	elseif savedIdle == '3' then
		if buffactive['shell'] then
			equip(sets.me.mdtShell)
		else
			equip(sets.me.mdt)
		end
	elseif savedIdle == '4' then
		equip(sets.me.kite)
	else
		add_to_chat(128, savedIdle)
		add_to_chat(128, "No Idle set found for idle variable")
	end 
end
...
function idle(pet)
    ...
    if meleeing and player.status=='Engaged' then   
        ...
        equip(sets.me.melee)        
    elseif favor and pet.isvalid then   
...
function self_command(command)
	...
	if command == 'idle' then								-- IDLE SET COMMANDS
		if savedIdle == '0' then 	-- Makes idle set REFRESH
			savedIdle = '1' 
			add_to_chat(158, "Refresh Mode")
		else				-- Makes idle set Gaiters
			savedIdle = '0'
			add_to_chat(158, "Movement Mode")
		end
		playerIdle(savedIdle)
	elseif command == 'pdt' then
		savedIdle = '2' 				-- Makes idle set PDT
		playerIdle(savedIdle)
		add_to_chat(158, "PDT Mode")
	elseif command == 'mdt' then
		savedIdle = '3'					-- Makes idle set MDT
		playerIdle(savedIdle)
		add_to_chat(158, "MDT Mode")
	elseif command == 'kite' then
		savedIdle = '4'
		playerIdle(savedIdle)
		add_to_chat(158, "Kite Mode")
	end
    ...
    if #commandArgs:split(' ') >= 2 then
		...
		if commandArgs[1] == 'avatar' then
			if commandArgs[2] then
				...
				equip(sets.avatar[mode])
				windower.add_to_chat(8,'----- Avatar '..mode..' mode! -----')
			end
